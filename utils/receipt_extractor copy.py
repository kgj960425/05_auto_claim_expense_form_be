...
def extract_merchant_name(text: str) -> Optional[str]:
    """
    가맹점명/상호/매장명 라벨 라인에서 값 가져오기.
    - 같은 줄 "라벨: 값" 형태 우선
    - 없으면 다음 비어있지 않은 줄에서 값 추정
    """
    lines = [l.strip() for l in text.splitlines()]
    label_pat = re.compile(r'^(?:가맹점명|가맹점|상호명|상호|매장명)\s*[:\-]?\s*(.*)$')
    ...
    for i, line in enumerate(lines):
        ...
        if same_line_val:
            ...
            # 2) 다음 줄 값(아래 3줄 내에서 첫 비어있지 않은 줄)
            cand = ""
            for j in range(i + 1, min(i + 4, len(lines))):
                nxt = lines[j].strip()
                if not nxt:
                    continue
                if label_pat.match(nxt):  # 라벨이면 스킵
                    continue
                cand = nxt
                break

        cand = _clean_merchant(cand)
        if _is_valid_merchant(cand):
            return cand

    # 백업: 상단 10줄에서 그럴듯한 후보
    for line in lines[:10]:
        cand = _clean_merchant(line)
        if _is_valid_merchant(cand):
            return cand
...
# ==== [교체] OCR dict 기반 가맹점명 추출 ====
def extract_merchant_name_from_ocr_data(
    ocr_data: Dict[str, Any],
    min_confidence: int = 30
) -> Optional[str]:
    """
    '가맹점명/가맹점/상호/상호명/매장명' 라벨을 찾아
    - 같은 줄 오른쪽 토큰들
    - 없으면 바로 아래 줄의 오른쪽 토큰들
    을 x좌표 순으로 결합해서 가맹점명을 만든다.
    """
    n = len(ocr_data['text'])

    def ok(i: int) -> bool:
        try:
            return int(ocr_data['conf'][i]) >= min_confidence and ocr_data['text'][i].strip() != ''
        except Exception:
            return False

    labels = {'가맹점명', '가맹점', '상호명', '상호', '매장명'}
    label_idx = None

    # 1) 라벨 박스 찾기 (OCR이 '가맹점명'을 '가맹점'+'명'으로 쪼개는 케이스 고려)
    for i in range(n):
        if not ok(i):
            continue
        t = ocr_data['text'][i].strip()
        if t in labels or ('가맹점' in t) or ('상호' in t) or ('매장명' in t):
            label_idx = i
            break
    ...
    ly = ocr_data['top'][label_idx]
    lx_end = ocr_data['left'][label_idx] + ocr_data['width'][label_idx]
    ...
    for i in range(n):
        ...
        if t in {':', '-', '=', '|'}:
            continue
        if abs(y - ly) <= 12:
            same_line.append((x, t))

    if same_line:
        same_line.sort(key=lambda z: z[0])
        cand = _clean_merchant(' '.join(t for _, t in same_line))
        if _is_valid_merchant(cand):
            return cand

    # 3) 바로 아래 줄(15~120px), 오른쪽
    next_line = []
    for i in range(n):
        if not ok(i):
            continue
        ...
        if 15 <= dy <= 120:
            ...
            if t in {':', '-', '=', '|'}:
                continue
            next_line.append((y, x, t))

    if next_line:
        next_line.sort(key=lambda z: (z[0], z[1]))
        top_y = next_line[0][0]
        group = [t for (y, x, t) in next_line if abs(y - top_y) <= 10]
        cand = _clean_merchant(' '.join(group))
        if _is_valid_merchant(cand):
            return cand
...
def _clean_merchant(s: str) -> str:
    # 라벨 단어가 앞에 섞인 경우 제거
    s = re.sub(r'^\s*(?:가맹점명|가맹점|상호명|상호|매장명)\s*[:\-]?\s*', '', s)
    # 한글/영문/숫자/공백/일부 기호만 허용 (㈜, (), -, &, . 등)
...
def _is_valid_merchant(s: str) -> bool:
    if not s or len(s) < 2:
        return False
    letters = re.sub(r'[^A-Za-z가-힣]', '', s)
    digits = re.sub(r'[^0-9]', '', s)
    if letters == '' and len(digits) > 0:  # 숫자만
        return False
    if s in {'영수증', '정상매입', '합계', '금액', '부가세', '일시불', '할부'}:
        return False
    return True