...
def run_ocr_on_pdfs(who :str, timestamp: str, job_uuid: str):
    ...
    # === 2️⃣ 결과 폴더 생성 ===
    # 출력 폴더명: static/temp/작업자_작업시작시간_UUID
    output_folder = f"static/temp/{who}_{timestamp}_{job_uuid}"
    ...
    # === 3️⃣ 선택된 PDF 파일들을 pdf 폴더에 복사 ===
    for pdf_path in pdf_paths:
        filename = os.path.basename(pdf_path)
        destination = os.path.join(pdf_folder, filename)
        shutil.copy2(pdf_path, destination)
        print(f"[저장 완료] {filename} -> {destination}")
...
if __name__ == "__main__":
    ...
    run_ocr_on_pdfs(who=who, timestamp=timestamp, job_uuid=job_uuid)
